...
if [[ $osVersion == *"Ubuntu 24"* ]]; then 
    ...
    apt-get update && apt-get -y upgrade && apt-get install python3-venv python3-dev pkg-config libxmlsec1-dev virtualenv redis-server postgresql nginx git gcc vim unzip imagemagick python3-certbot-nginx certbot wget xz-utils python3-lxml python3-xmlsec -y
else
    echo "This script is tested for Ubuntu 24 only, if you want to try MediaCMS on another system you have to perform the manual installation"
    exit
fi
