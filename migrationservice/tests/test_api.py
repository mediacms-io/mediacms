...
class TestCrud(ApiTestCase):
    ...
    def test_an_lti_platform_is_refused_when_the_portal_has_no_lti(self):
        payload = {
            "name": "Test run",
            "provider": "kaltura",
            "connection": CONNECTION,
            "options": {"source_category_ids": "1", "lti_platform_id": "1"},
        }
        with override_settings(USE_LTI=False):
            response = self.client.post(API, data=json.dumps(payload), content_type="application/json")
        self.assertEqual(response.status_code, 400, response.content)

    def test_an_lti_platform_that_does_not_exist_is_refused(self):
        payload = {
            "name": "Test run",
            "provider": "kaltura",
            "connection": CONNECTION,
            "options": {"source_category_ids": "1", "lti_platform_id": "4242"},
        }
        with override_settings(USE_LTI=True):
            response = self.client.post(API, data=json.dumps(payload), content_type="application/json")
        self.assertEqual(response.status_code, 400, response.content)
...
class TestConnectionCheck(ApiTestCase):
    ...
    def test_a_source_that_cannot_be_reached_reports_instead_of_raising(self):
        payload = {
            "provider": "panopto",
            "connection": {"service_url": "https://panopto.invalid", "client_id": "a", "client_secret": "b", "username": "u", "password": "p"},
        }
        response = self.client.post(f"{API}check_connection/", data=json.dumps(payload), content_type="application/json")
        self.assertEqual(response.status_code, 200)
...
class TestOneUserChoice(ApiTestCase):
    """Whose media comes over is one question with three answers, not three switches."""
    ...
    def save(self, **options):
        payload = {
            ...
            "provider": "kaltura",
            "connection": CONNECTION,
            "options": dict({"source_category_ids": "1"}, **options),
        }
        response = self.client.post(API, data=json.dumps(payload), content_type="application/json")
        self.assertEqual(response.status_code, 201, response.content)
        service = MigrationService.objects.get(pk=response.json()["id"])
        return {key: service.options.get(key) for key in ("migrate_all_users", "restrict_to_users", "create_users")}

    def setUp(self):
        super().setUp()
        self.login(self.admin)

    def test_the_narrowest_answer_wins_when_a_payload_names_two(self):
        self.assertEqual(
            self.save(migrate_all_users=True, create_users=True),
            {"migrate_all_users": False, "restrict_to_users": False, "create_users": True},
        )

    def test_naming_none_of_them_migrates_everybody(self):
        self.assertEqual(
            self.save(migrate_all_users=False, create_users=False, restrict_to_users=False),
            {"migrate_all_users": True, "restrict_to_users": False, "create_users": False},
        )

    def test_the_one_chosen_is_the_one_stored(self):
        self.assertEqual(
            self.save(migrate_all_users=False, create_users=True),
            {"migrate_all_users": False, "restrict_to_users": False, "create_users": True},
        )

    def test_whoever_saved_it_is_remembered(self):
        payload = {"name": "Kaltura", "provider": "kaltura", "connection": CONNECTION, "options": {"source_category_ids": "1"}}
        response = self.client.post(API, data=json.dumps(payload), content_type="application/json")
        service = MigrationService.objects.get(pk=response.json()["id"])
        self.assertEqual(service.options["initiated_by"], self.admin.username)
