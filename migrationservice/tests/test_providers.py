...
class TestProviderRegistry(TestCase):
    ...
    def test_every_registered_provider_is_implemented(self):
        for name in ["kaltura", "panopto", "youtube"]:
            self.assertTrue(get_provider_class(name).implemented, name)

    def test_the_contract_itself_raises_until_a_provider_answers_it(self):
        provider = BaseProvider({}, {})
        for call in (provider.check_connection, lambda: provider.list_page("media", {}, 10), lambda: provider.fetch_media("x")):
