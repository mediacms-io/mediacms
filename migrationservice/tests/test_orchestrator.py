...
from migrationservice.tasks import abort_migration, finish_migration
...
class TestFullRun(OrchestratorTestCase):
    ...
    def test_disabled_phases_are_skipped(self):
        ...
        self.assertEqual(MigrationRecord.objects.filter(service=service, object_type="category").count(), 1)
        self.assertEqual(Media.objects.count(), 2)
...
class TestPhases(OrchestratorTestCase):
    """migrate_all_users adds a sweeping phase before media. With it off the owners are
    still created, just on demand. Categories always sweep, scoped by the chosen ones.
    """
    ...
    def test_all_categories_are_migrated_even_with_no_media(self):
        ...
        # the sweep is no longer optional, so a gallery holding nothing still comes over
