...
def make_provider():
    ...
    provider.categories = {
        ...
        "8811": {
            "id": "8811",
            "name": "Engineering",
            "fullName": "MediaSpace>site>galleries>Engineering",
            ...
            "categories": [
                {
                    ...
                    "privacy": 1,
                    "fullName": "MediaSpace>site>galleries>Engineering>Electronics",
                    "fullIds": "771961>771967>771968>8811>8812",
                }
            ],
        }
    }
...
class TestImportMediaEntry(TestCase):
    ...
    def test_the_category_is_created_and_linked(self):
        media = import_media_entry(self.service, self.provider, "1_a")
        self.assertIn("Electronics", [category.title for category in media.category.all()])
        self.assertEqual(Category.objects.filter(title="Electronics").count(), 1)

    def test_the_parent_category_holds_the_entry_as_well(self):
        media = import_media_entry(self.service, self.provider, "1_a")
        self.assertEqual(sorted(category.title for category in media.category.all()), ["Electronics", "Engineering"])

    def test_the_rollup_can_be_switched_off(self):
        service = make_service(rollup_subcategories=False)
    ...
    def test_a_private_entry_is_not_rolled_up_into_an_access_controlled_parent(self):
        self.provider.categories["8811"]["privacy"] = 3
        self.provider.categories["8812"]["privacy"] = 3
        self.provider.media["1_a"]["categories"][0]["privacy"] = 3
    ...
    def test_flavors_become_encodings_on_active_profiles(self):
        media = import_media_entry(self.service, self.provider, "1_a")
        encodings = Encoding.objects.filter(media=media).exclude(profile__name=PREVIEW_PROFILE_NAME)
        # the two flavors, plus the original registered as one so the player offers its
        # resolution too. The fixture file is 240p whatever the flavor metadata claims.
        self.assertEqual(encodings.count(), 3)
        self.assertEqual(sorted(encoding.profile.resolution for encoding in encodings), [240, 360, 720])
    ...
    def test_the_only_encoding_queued_is_the_hover_preview(self):
        with mock.patch("files.tasks.encode_media") as encode:
            import_media_entry(self.service, self.provider, "1_a")

        queued = [call.args[1] for call in encode.apply_async.call_args_list if len(call.args) > 1]
        profiles = EncodeProfile.objects.filter(id__in=[ids[1] for ids in queued]) if queued else EncodeProfile.objects.none()
        self.assertTrue(all(profile.name == PREVIEW_PROFILE_NAME for profile in profiles), [profile.name for profile in profiles])
    ...
    def test_the_original_file_is_used_when_no_flavor_survives(self):
        ...
        media = import_media_entry(self.service, self.provider, "1_a")
        media.refresh_from_db()
        self.assertEqual(Encoding.objects.filter(media=media).exclude(profile__name=PREVIEW_PROFILE_NAME).count(), 1)
        self.assertEqual(media.encoding_status, "success")
