...
def make_service(**options):
    defaults = {"fallback_username": "admin", "source_category_ids": "8812"}
...
class TestScheduledRerun(TestCase):
    """A schedule set on a finished migration means "sweep it again then", which is the
    re-run the list page offers, not a start it would be refused.
    """
    ...
    def test_a_completed_migration_is_run_again(self):
        when = in_hours(-1)
        service = make_service(schedule_enabled=True, scheduled_at=when)
        MigrationService.objects.filter(pk=service.pk).update(status="success", cursor={"page": 4})

        with mock.patch("migrationservice.tasks.run_migration.apply_async") as queued:
            self.assertTrue(run_scheduled_migration(service.pk, when))
    ...
    def test_a_running_migration_is_left_alone(self):
        when = in_hours(-1)
        service = make_service(schedule_enabled=True, scheduled_at=when)
        MigrationService.objects.filter(pk=service.pk).update(status="running")
        self.assertFalse(run_scheduled_migration(service.pk, when))
...
class TestWeekends(TestCase):
    WINDOW = {"quiet_hours_enabled": True, "quiet_from": "08:00", "quiet_to": "17:00"}

    def at(self, day, hour):
        return timezone.make_aware(datetime(2026, 9, day, hour, 0), timezone.get_current_timezone())

    def test_the_window_holds_on_a_weekday_either_way(self):
        wednesday = self.at(23, 10)
        self.assertTrue(in_quiet_window(self.WINDOW, wednesday))
        self.assertTrue(in_quiet_window(dict(self.WINDOW, run_during_weekends=True), wednesday))

    def test_the_window_is_ignored_on_saturday_and_sunday(self):
        for day in (26, 27):
            self.assertTrue(in_quiet_window(self.WINDOW, self.at(day, 10)))
            self.assertFalse(in_quiet_window(dict(self.WINDOW, run_during_weekends=True), self.at(day, 10)))


class TestServerTimeEndpoint(TestCase):
    def setUp(self):
        password = "this_is_a_fake_password"
        create_account(username="super", password=password, is_superuser=True)
        self.client.login(username="super", password=password)

    def test_it_reports_the_portal_clock_not_utc(self):
        response = self.client.get(reverse("migrations-server-time"))
