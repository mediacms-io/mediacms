...
class TestRoleMapping(TestCase):
    ...
    def test_a_portal_that_reports_the_label_too_still_matches(self):
        self.assertEqual(mediacms_role("", "Manager (KMC)"), "admin")

    def test_the_shipped_system_roles(self):
        self.assertEqual(mediacms_role("", "adminRole"), "advancedUser")
        self.assertEqual(mediacms_role("", "unmoderatedAdminRole"), "editor")
        # listed so it is visible in the form, but it is a plain user
        self.assertEqual(mediacms_role("", "privateOnlyRole"), "")

    def test_a_qualifier_does_not_collapse_distinct_roles(self):
        rows = [{"id": "", "name": "Manager (KMC)", "role": "admin"}]
        self.assertEqual(mediacms_role("", "Site Manager", rows), "")
        self.assertEqual(mediacms_role("", "Player Designer"), "manager")
        self.assertEqual(mediacms_role("", "Manager"), "admin")
        self.assertEqual(mediacms_role("", "Publisher Administrator"), "admin")
...
class TestAncestorCategories(TestCase):
    def test_a_kms_path_gives_the_categories_above_it(self):
        self.assertEqual(
            ancestor_categories(
                "MediaSpace>site>galleries>Guides>Kaltura>Interactive Video",
                "771961>771967>771968>839899>839903>839905",
            ),
            [("839899", "MediaSpace>site>galleries>Guides"), ("839903", "MediaSpace>site>galleries>Guides>Kaltura")],
        )

    def test_the_scaffolding_and_the_instance_root_are_never_parents(self):
        self.assertEqual(ancestor_categories("MediaSpace>site>channels>Physics", "771961>771966>771969>839921"), [])

    def test_a_portal_with_no_kms_tree_keeps_its_own_hierarchy(self):
        self.assertEqual(ancestor_categories("Engineering>Electronics", "10>11"), [("10", "Engineering")])

    def test_an_incontext_child_rolls_up_to_its_course_and_no_further(self):
        self.assertEqual(
            ancestor_categories("moodle>site>channels>14>InContext", "1>2>3>4>5"),
            [("4", "moodle>site>channels>14")],
        )

    def test_incontext_is_never_offered_as_a_parent_itself(self):
        self.assertEqual(ancestor_categories("moodle>site>channels>14>InContext>Week 1", "1>2>3>4>5>6"), [("4", "moodle>site>channels>14")])

    def test_a_top_level_category_has_no_parents(self):
        self.assertEqual(ancestor_categories("MediaSpace", "771961"), [])

    def test_ids_that_do_not_line_up_with_the_path_are_refused(self):
        self.assertEqual(ancestor_categories("A>B>C", "1>2"), [])
        self.assertEqual(ancestor_categories("A>B", ""), [])


class TestNeedsRbacGroup(TestCase):
    """Both of KMS's restriction fields, as a live portal reports them."""

    def test_no_restriction_on_either_field_needs_no_group(self):
        self.assertFalse(needs_rbac_group({"privacy": 1, "appearInList": 1, "privacyContexts": "MediaSpace"}))

    def test_a_private_listing_needs_a_group_even_with_a_public_library(self):
        self.assertTrue(needs_rbac_group({"privacy": 1, "appearInList": 3, "privacyContexts": "MediaSpace"}))

    def test_requires_authentication_needs_a_group(self):
        self.assertTrue(needs_rbac_group({"privacy": 2, "appearInList": 1, "privacyContexts": "MediaSpace"}))

    def test_members_only_needs_a_group(self):
        self.assertTrue(needs_rbac_group({"privacy": 3, "appearInList": 3, "privacyContexts": "MediaSpace"}))

    def test_an_unrecognised_privacy_value_needs_a_group(self):
        self.assertTrue(needs_rbac_group({"privacy": 99, "appearInList": 1, "privacyContexts": "MediaSpace"}))

    def test_an_lms_tree_has_no_permissions_to_carry(self):
        self.assertFalse(needs_rbac_group({"privacy": 3, "appearInList": 3, "privacyContexts": ""}))
...
class TestTreeKind(TestCase):
    """LMS and KMS are told apart by privacyContexts, which is Kaltura's own switch for
    whether the permission fields on a category mean anything at all.
    """

    def test_a_category_inside_a_privacy_context_is_kms(self):
        self.assertEqual(category_tree_kind({"privacyContexts": "MediaSpace"}), KMS_TREE)

    def test_a_category_with_no_privacy_context_is_lms(self):
    ...
    def test_lms_permission_fields_are_inert(self):
        # outside a privacy context Kaltura serves this to anyone, whatever privacy says
        category = {"privacyContexts": "", "privacy": PRIVACY_MEMBERS_ONLY, "contributionPolicy": CONTRIBUTION_MEMBERS}
        self.assertEqual(category_type(category), CATEGORY_PUBLIC)

    def test_the_same_fields_inside_a_context_are_honoured(self):
        category = {"privacyContexts": "MediaSpace", "privacy": PRIVACY_MEMBERS_ONLY, "contributionPolicy": CONTRIBUTION_MEMBERS}
        self.assertEqual(category_type(category), CATEGORY_PRIVATE)


class TestIgnoredCategories(TestCase):
    def test_incontext_in_an_lms_tree_is_ignored(self):
        category = {"privacyContexts": "", "name": "InContext", "fullName": "lti_x>site>channels>Course 1>InContext"}
        self.assertTrue(is_ignored_category(category))

    def test_incontext_in_a_kms_tree_is_a_real_category(self):
        # somebody named a gallery this. It is theirs, not an integration's plumbing.
        category = {"privacyContexts": "MediaSpace", "name": "InContext", "fullName": "MediaSpace>site>galleries>InContext"}
        self.assertFalse(is_ignored_category(category))

    def test_an_ordinary_lms_course_is_not_ignored(self):
        category = {"privacyContexts": "", "name": "Course 1", "fullName": "lti_x>site>channels>Course 1"}
        self.assertFalse(is_ignored_category(category))


class TestSelectableCategories(TestCase):
    def test_only_top_level_is_offered(self):
        self.assertTrue(is_selectable_category("MediaSpace"))
        self.assertTrue(is_selectable_category("ltigeneric_8yrU6"))
        self.assertFalse(is_selectable_category("MediaSpace>site>channels>Physics"))
        self.assertFalse(is_selectable_category("MediaSpace>site"))
        self.assertFalse(is_selectable_category(""))


class TestInstanceRoots(TestCase):
    def test_a_root_with_a_site_child_is_scaffolding(self):
        paths = {"MediaSpace", "MediaSpace>site", "MediaSpace>site>galleries>Engineering"}
        self.assertTrue(is_instance_root("MediaSpace", paths))

    def test_a_plain_portals_top_level_category_is_real(self):
        paths = {"Engineering", "Engineering>2024"}
        self.assertFalse(is_instance_root("Engineering", paths))

    def test_anything_below_top_level_is_never_a_root(self):
        paths = {"MediaSpace", "MediaSpace>site>galleries>Engineering"}
        self.assertFalse(is_instance_root("MediaSpace>site>galleries>Engineering", paths))
