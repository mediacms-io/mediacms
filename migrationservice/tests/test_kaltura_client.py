...
class TestKalturaClient(TestCase):
    ...
    def test_network_errors_are_retried_then_raised(self):
        ...
        client.ks = "the-ks"
        client.ks_issued_at = 9999999999
        # pinned, so the assertion is about retrying exactly as many times as configured
        # rather than about whatever the shipped default happens to be
        with override_settings(MIGRATION_MAX_RETRIES=3):
...
class TestKalturaCheckConnection(TestCase):
    ...
    def test_the_entry_count_honours_the_entry_filter(self):
        ...
        # the first two calls are the session, so the entry count is the third
        entry_call = post.call_args_list[2]
...
class TestKmsRole(TestCase):
    """MediaSpace keeps its own role in the user schema metadata, keyed on the puser id"""

    def provider(self):
        return KalturaProvider({"service_url": "https://kaltura.example.edu", "partner_id": "342", "app_token_id": "atok", "app_token": "x"}, {})

    def test_the_role_is_read_out_of_the_metadata_xml(self):
        provider = self.provider()
        answer = {"totalCount": 1, "objects": [{"objectId": "jdoe", "xml": "<metadata><role>adminRole</role></metadata>"}]}
        with mock.patch.object(provider.client, "call", return_value=answer) as called:
            self.assertEqual(provider.kms_role("jdoe"), "adminRole")
        # pinned on the object type only: adding the profile id returns nothing
        kfilter = called.call_args.kwargs["filter"]
        self.assertEqual(kfilter["metadataObjectTypeEqual"], "3")
        self.assertEqual(kfilter["objectIdEqual"], "jdoe")
        self.assertNotIn("metadataProfileIdEqual", kfilter)

    def test_a_user_without_one_gets_nothing(self):
        provider = self.provider()
        for answer in ({"totalCount": 0, "objects": []}, {"objects": [{"xml": "<metadata></metadata>"}]}):
            with mock.patch.object(provider.client, "call", return_value=answer):
                self.assertEqual(provider.kms_role("jdoe"), "")

    def test_a_metadata_error_is_not_fatal(self):
        provider = self.provider()
        with mock.patch.object(provider.client, "call", side_effect=KalturaAPIError("NOPE", "no")):
            self.assertEqual(provider.kms_role("jdoe"), "")

    def test_a_console_role_is_used_and_no_metadata_call_is_made(self):
        provider = self.provider()
        answers = [{"id": "jdoe", "roleIds": "1691"}, {"objects": [{"systemName": "Manager"}]}]
        with mock.patch.object(provider.client, "call", side_effect=answers):
            with mock.patch.object(provider, "kms_role") as kms:
                payload = provider.fetch_user("jdoe")
        self.assertEqual(payload["roleName"], "Manager")
        self.assertEqual(kms.call_count, 0)

    def test_the_mediaspace_role_fills_in_when_there_is_no_console_role(self):
        provider = self.provider()
        with mock.patch.object(provider.client, "call", return_value={"id": "jdoe", "roleIds": ""}):
            with mock.patch.object(provider, "kms_role", return_value="unmoderatedAdminRole") as kms:
                payload = provider.fetch_user("jdoe")
        self.assertEqual(payload["roleName"], "unmoderatedAdminRole")
        self.assertEqual(kms.call_count, 1)
