...
from files.models.utils import generate_uid
...
from .scheduling import parse_scheduled_at, scheduled_at_text
    ...
    for source_category in data.get("categories") or []:
        ...
        if is_ignored_category(source_category):
            ...
            if not source_category_id:
                continue
        elif not is_importable_category(source_category.get("fullName")):
            continue
...
PHASES = ["users", "groups", "categories", "media"]
...
PHASE_OPTION = {"users": "migrate_all_users", "groups": "migrate_groups", "categories": None, "media": None}

RECORD_TYPE = {"users": "user", "groups": "group", "categories": "category", "media": "media"}

IMPORTERS = {"users": import_user, "groups": import_group, "categories": import_category, "media": import_media_entry}
...
def record_discovery_totals(service, provider, force=False):
    ...
    if getattr(provider, "migrates_nothing", lambda: False)():
        service.append_log("no categories are selected, so no media will be migrated")
