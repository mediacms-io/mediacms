...
# The picker offers this alongside the real top level categories. Kaltura entries very often
# belong to no category at all, and no category filter can ever reach them, so without a row
# of their own they are invisible and silently left behind.
UNCATEGORISED_ID = "__none__"
...
class KalturaProvider(BaseProvider):
    ...
    retired_options = ("create_categories", "map_permissions", "migrate_all_categories")
    ...
    def check_connection(self):
        ...
        # the media phase is a union of sweeps, so the headline number has to be counted the
        # same way rather than taken from one filter. Sweeps can overlap, so this is an
        # upper bound on distinct entries and the run may import slightly fewer
        sweeps = self.media_sweeps()
        result["stats"]["entries"] = sum(self.client.count("media", sweep) for sweep in sweeps)
        result["stats"]["entries_are_an_upper_bound"] = len(sweeps) > 1
        if self.migrates_nothing():
    ...
    def migrates_nothing(self):
        """Whether this migration has been narrowed down to nothing at all"""
        return not self.media_sweeps()

    def restricted_category_ids(self):
        """The real categories this migration is limited to.

        The uncategorised marker is not one of them: it is a sweep of its own, and passing
        it to categoryAncestorIdIn would be asking Kaltura about a category id that has
        never existed.
        """
        return [identifier for identifier in parse_comma_list(self.options.get("source_category_ids")) if identifier != UNCATEGORISED_ID]

    def includes_uncategorised(self):
        """Whether entries belonging to no category at all were asked for"""
        return UNCATEGORISED_ID in parse_comma_list(self.options.get("source_category_ids"))

    def media_sweeps(self):
        """The filters the media phase walks, in order, whose results are a union.

        A union rather than one filter because Kaltura ANDs every field in a filter
        together, and the question being asked is an or: the media in these categories, plus
        whatever else these named people own. Overlap between sweeps costs one query per
        entry, since the importer returns early on anything it has already brought in.
        """
        base = self._entry_filter()
        sweeps = []
        ...
        if category_ids:
            ...
            sweeps.append(dict(base, categoryAncestorIdIn=",".join(category_ids)))

        if self.includes_uncategorised():
            # categoriesIdsEmpty is honoured: on the test portal it answered 17 of 74, and a
            # deliberately unsupported field answered 74
            sweeps.append(dict(base, categoriesIdsEmpty=1))

        user_ids = self.restricted_user_ids()
        if user_ids:
            # everything these people own, wherever it sits, which is the "plus" half of
            # the union and the reason their media is not lost to a category they never used
            sweeps.append(dict(base, userIdIn=",".join(user_ids)))
    ...
    def list_page(self, phase, cursor, page_size):
        # asked for particular categories and named none: there is nothing to sweep, and no
        # filter is sent to Kaltura that it might answer by returning everything
        if self.migrates_nothing() and phase in ("media", "categories"):
            return [], cursor

        if phase == "media":
            return self._list_media_page(cursor, page_size)
    ...
    def _list_media_page(self, cursor, page_size):
        """One page of the media phase, walking each sweep in turn.

        The cursor carries which sweep is being walked alongside that sweep's own position,
        so a migration that is paused in the middle of the second sweep resumes there rather
        than starting the first one again. An exhausted sweep hands over to the next with a
        fresh position, and an empty list once past the last sweep is what ends the phase.
        """
        sweeps = self.media_sweeps()
        cursor = dict(cursor or {})
        index = cursor.get("sweep") or 0

        while index < len(sweeps):
            entries, next_position = self.client.list_entries_page(cursor.get("position") or {}, page_size, sweeps[index])
            if entries:
                return [str(entry["id"]) for entry in entries], {"sweep": index, "position": next_position}
            index += 1
            cursor = {"sweep": index, "position": {}}

        return [], {"sweep": index, "position": {}}

    def _worth_importing(self, category):
        """Whether one category from the walk becomes a MediaCMS category"""
        full_name = category.get("fullName") or ""
        if not category.get("id"):
            return False
        if is_ignored_category(category):
            return False
        if is_instance_root(full_name, self._all_category_paths()):
            return False
    ...
    def _list_importable_categories(self, cursor, page_size):
        ...
        while True:
            ...
            keep = [str(category["id"]) for category in categories if self._worth_importing(category)]
            if keep:
    ...
    def list_categories(self):
        ...
        chosen = sorted(chosen, key=lambda category: category["fullName"])

        # last, and with a count of its own: entries belonging to no category are invisible
        # to every category filter, so without a row here they are quietly left behind
        chosen.append(
            {
                "id": UNCATEGORISED_ID,
                "name": "(no category)",
                "fullName": "",
                "kind": "",
                "entries": self.client.count("media", {"categoriesIdsEmpty": 1}),
            }
        )
        return chosen
