...
class MigrationServiceViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        options = dict(serializer.validated_data.get("options") or {})
        options["initiated_by"] = self.request.user.get_username()

        # arming happens on save, not on Start: nobody has to be here when the time comes
        schedule_migration(serializer.save(options=options))
    ...
    @action(detail=False, methods=["get"], url_path="lti_platforms")
    def lti_platforms(self, request):
        """The LTI platforms a migrated LMS course can be wired to.

        Empty when the portal has no LTI, which is how the form knows not to offer it.
        """
        if not getattr(settings, "USE_LTI", False):
            return Response([])

        return Response([{"id": str(platform.pk), "name": platform.name} for platform in LTIPlatform.objects.order_by("name")])
