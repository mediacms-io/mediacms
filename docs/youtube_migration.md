...
## When YouTube will not answer

From a laptop, public video needs no credentials. From a server, expect to need cookies:
**YouTube answers a request from a datacentre address with a bot check** whether the video is
public or not, and no amount of retrying changes it.

**Cookies file contents** is the only field that speaks to this. It is also needed for
private, members-only or age-gated video. It is encrypted at rest and the API never returns
it.

How the jar is exported decides whether it works:

1. A **separate browser profile**, or a private window with the extension allowed in it.
2. Sign in there with an account you can afford to hand over: a jar is a live session.
3. Export with a cookies.txt extension, in Netscape format.
4. Paste it in, save, **Test connection**.
5. Then leave that profile alone. Do not browse YouTube in it, and do not log out.

Step 5 decides it. `__Secure-1PSIDTS` and `__Secure-3PSIDTS` rotate every few hours and stay
bound to the session that exported them, so a jar from a browser that keeps running is
superseded within minutes. YouTube then answers **"The page needs to be reloaded"** — which is
why a jar with all 22 cookies and expiry dates months away can still break requests that
would have succeeded without it. A migration tries anonymously first for that reason.

When neither works, the address is the problem and the portal cannot route around it.
Reproduce it over ssh with the call the migration makes:

```bash
yt-dlp --flat-playlist --skip-download --print id "https://www.youtube.com/watch?v=VIDEO_ID"
```

A bot check there, with no cookies, is YouTube refusing that machine. What is left is to fetch
from somewhere it is not challenged: another host, a proxy in front of yt-dlp, or `--cookies`
outside the app. yt-dlp moves fast on this, so update it before concluding anything.
