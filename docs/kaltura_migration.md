...
**What you pick is added to what the user options bring, not intersected with it.** Picking a
category and also naming specific users gives you everything in that category, plus
everything those people own wherever it happens to sit. An intersection would have been the
other reading, and it is the wrong one: it quietly drops a named person's media the moment it
lives outside the chosen category.
...
**Entries in no category get a row of their own.** A Kaltura entry belongs to a category only
if somebody published it to one, and a great many never are: on the portal this was built
against, 17 of 74 entries belonged to no category at all, owned by nine different people and
holding real content. No category filter can ever reach them, so the picker lists
**(no category)** with its own count, and ticking it runs a sweep of its own. Those entries
arrive **private**, since the source published them nowhere.

Because the picker is a union of sweeps that can overlap, **Test connection** reports "up to"
a number rather than an exact one when more than one sweep is in play. An entry found twice
is imported once: the importer returns early on anything it has already brought in.

**At least one has to be ticked**, and (no category) counts. Save draft and Save and start are
both refused until then, in the form and again on the server. Existing migrations saved before this carry a retired
