...
const TYPES = [
  ...
  { key: 'group', label: translateString('Groups') },
  { key: 'category', label: translateString('Categories') },
  { key: 'media', label: translateString('Media') },
  { key: 'playlist', label: translateString('Playlists') },
...
export class MigrationDetailPage extends Page {
  ...
  componentDidMount() {
    getMigration(this.state.id)
      ...
      .catch((error) => {
        const missing = error && error.response && 404 === error.response.status;
        if (missing) {
          // nothing to poll for: without this the page asks a dead id every few seconds
          clearInterval(this.timer);
        }
        this.safeSetState({
          error: translateString(missing ? 'That migration no longer exists.' : 'Could not load migration'),
          missing: !!missing,
        });
      });
    this.poll();
  ...
  poll() {
    if (this.state.missing) {
      // the migration is gone; the in flight requests from before must not talk over it
      return;
    }
    getProgress(this.state.id)
      ...
      .catch(() => this.state.missing || this.safeSetState({ error: translateString('Could not load progress') }));
  ...
  pageContent() {
    ...
    if (!migration || !progress) {
      ...
      if (this.state.error && (this.state.missing || !migration)) {
        return (
