...
const CONNECTION_FIELDS = {
  ...
  youtube: [
    ...
    },
    {
      ...
      type: 'textarea',
      wide: true,
      help: 'Only for private or members-only video. Export cookies.txt from a signed-in browser. Public video needs nothing here, and cookies expire.',
    },
  ],
};
...
const OPTIONS = [
  ...
  // setting of its own: every media goes to the fallback owner.
  {
    ...
    help: 'Only create specific users listed below. Only media (entries) of these users will be migrated.',
  ...
  },
  {
    ...
    help: 'Static playlists become MediaCMS playlists, in the same order. Media a playlist names is fetched if the run did not otherwise reach it, and its owner created. Dynamic playlists are a saved query, not a list, and are skipped.',
  },
  {
    key: 'migrate_groups',
...
function optionsFor(provider) {
  return 'youtube' === provider ? YOUTUBE_OPTIONS : OPTIONS;
}
    ...
    return (
      <>
        <p className="migration-edit-help">
          {translateString(
            'Load and select root categories. For each root category selected, subordinated site categories and channels will be migrated. Media (entries) associated with each category or channel, will likewise be associated with the corresponding category in MediaCMS.'
          )}
        </p>
        ...
        <div className="migration-edit-check">
          <button onClick={this.onLoadCategoriesClick} disabled={this.state.loadingCategories || !this.connectionIsComplete()}>
            {this.state.loadingCategories
              ...
              : translateString('Load root categories from source')}
          </button>
      ...
      <>
        ...
        <div className="migration-edit-options">
          <label>
            <input
              type="checkbox"
              checked={true === options.quiet_hours_enabled}
              onChange={(e) => this.setOptionValue('quiet_hours_enabled', e.target.checked)}
            />
            <span className="migration-edit-option-text">
              {translateString('Do not run between')}
              <span className="migration-edit-help">
                {translateString(
                  'The migration waits while inside these hours and carries on by itself afterwards. Portal timezone. An end before the start runs over midnight.'
                )}
              </span>
            </span>
          </label>
        ...
        {true === options.quiet_hours_enabled ? (
        ...
        {scheduled ? (
          <label className="migration-edit-name">
            ...
            />
            <span className="migration-edit-help">
              {this.scheduleProblem() || translateString('Saving is enough.')}
            </span>
          </label>
        ) : null}
      </>
          ...
          <>
            ...
            <div className="migration-edit-actions">
              ...
              <span className="migration-edit-note">
                {saveProblem || this.scheduleNote() || translateString('Credentials are stored encrypted')}
              </span>
            </div>
          </>
        ) : null}
