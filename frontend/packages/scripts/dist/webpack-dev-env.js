...
function htmlPluginDataFunction(pluginData, options, callback) {
  pluginData.html = prettify(
    options.replace.reduce(function (res, item) {
      return res.replace(item instanceof RegExp ? new RegExp(item, 'gi') : item, '');
    }, pluginData.html) /*,
    options.config*/
  );
...
var MyHtmlBeautifyWebpackPlugin = /** @class */ (function () {
  ...
  MyHtmlBeautifyWebpackPlugin.prototype.apply = function (compiler) {
    ...
    function tapHookCallback(compilation) {
      return HtmlWebpackPlugin$1.getHooks(compilation).beforeEmit.tapAsync(
...
var sitemapTemplate = ejs.compile(fs.readFileSync(sitemapTemplatePath, 'utf8'), {
  root: [templatePath],
    ...
    if ('development' === env) {
      ...
      ret.push(
        new HtmlWebpackPlugin(
          __assign({ template: path.resolve(__dirname, '../templates/index.ejs'), hash: false, chunks: [k] }, pages[k])
        )
      );
    }
  }
...
function generateConfig(env, config) {
  ...
  var pages = formatPagesConfig(
    { title: '', filename: '', render: '', html: config.html, window: config.window },
    __assign(__assign({}, configPages), defPages)
  );
  var ret = {
    entry: webpackEntry(env, srcDir, pages),
    output:
      'development' === env
        ? webpackOutput(env, srcDir, void 0, void 0, void 0)
        : webpackOutput(env, buildDir, jsbuild, void 0, void 0),
...
function analyzer(analyzerOptions) {
  if (analyzerOptions === void 0) {
    analyzerOptions = defaultOptions$2;
  }
...
function build(buildOptions) {
  if (buildOptions === void 0) {
    buildOptions = defaultOptions$1;
  }
...
function dev(devOptions) {
  if (devOptions === void 0) {
    devOptions = defaultOptions;
  }
  ...
  var serverOptions = {
    static: {
      directory: options.config.src, // replaces contentBase iin v44
    },
    compress: true,
    hot: true,
    watchFiles: [options.config.src],
    host: options.host,
    port: options.port,
  };
